...
<template>
  <main class="mx-auto flex min-h-screen max-w-md flex-col justify-center px-4 py-10">
    ...
    <div class="mb-8 text-center">
      ...
      <p class="mt-2 text-sm text-mist">Start tracking the games you want to play.</p>
    </div>
    ...
    >
      ...
      <p v-if="info" class="mb-4 rounded-lg border border-accent/40 bg-accent/10 px-3 py-2 text-sm text-accent">
        {{ info }}
      </p>
    ...
    <p class="mt-6 text-center text-sm text-mist">
      ...
      <RouterLink class="text-accent hover:underline" :to="{ name: 'login' }">Sign in</RouterLink>
    </p>
  </main>
</template>
